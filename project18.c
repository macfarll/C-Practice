...
int main() {
    ...
    int iter_row,iter_val,iter_col,parent1,parent2,i;
    for (i=1;i<15;i++) {
        iter_row=14-i;
        for (iter_val=0;iter_val<iter_row+1;iter_val++) {
            parent1=arr[iter_row+1][iter_val];
            parent2=arr[iter_row+1][iter_val+1];
            if (parent1>parent2) {
                arr[iter_row][iter_val]+=parent1;
            }
            else {
                arr[iter_row][iter_val]+=parent2;
            }
        }
    }
    printf("Value for the maximum path is: %d\n",arr[0][0]);
}
/*
